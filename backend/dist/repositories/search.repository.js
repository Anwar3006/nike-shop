...
import { popular_searches, search_clicks, search_queries, } from "../models/search.model.js";
...
export const SearchRepository = {
    async getSearchResults(query) {
        ...
        const searchConditions = and(...searchWords.map((word) => ilike(shoes.name, `%${word.toLowerCase()}%`)));
        try {
    ...
    async recordSearchQuery(query, results_count, userId, ip_address) {
    ...
    async upsertPopularSearch(query) {
        // This will not work until the user creates the `popular_searches` table.
        try {
            await db
