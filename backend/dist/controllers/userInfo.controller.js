...
export const UserInfoController = {
    getUserInfo: catchAsync(async (req, res, next) => {
        ...
        if (!userId)
            return next(new AppError("UserId not Found", 401));
    ...
    updateInfo: catchAsync(async (req, res, next) => {
        const userId = req?.user?.id;
        if (!userId)
            return new AppError("UserId not Found", 401);
        const { firstName, lastName, phone, dob } = req.body;
        if (!firstName || !lastName || !phone || !dob) {
            return next(new AppError("All Fields are required", 404));
        }
        const name = firstName + " " + lastName;
        const updatedUserInfo = await UserInfoService.updateInfo(userId, name, phone, dob);
        res.status(200).json({
            status: "success",
            data: updatedUserInfo,
        });
    }),
    upsertAddress: catchAsync(async (req, res, next) => {
        const userId = req?.user?.id;
        if (!userId)
            return new AppError("UserId not Found", 401);
        const insertedAddress = await UserInfoService.upsertAddress(userId, req.body);
        if (!insertedAddress) {
    ...
    deleteAddress: catchAsync(async (req, res, next) => {
        ...
        const deletedAddress = await UserInfoService.deleteAddress(userId, addressId);
        if (!deletedAddress) {
