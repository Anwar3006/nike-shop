...
export const search_queries = pgTable("search_queries", {
  query_id: text("query_id").primaryKey().default(`gen_random_uuid()`),
...
export const search_clicks = pgTable("search_clicks", {
  ...
  query_id: text("query_id").references(() => search_queries.query_id),
...
export const popular_searches = pgTable(
  ...
  (table) => ({
    uniqueQuery: unique("unique_query").on(table.query),
  })
);

// Relations
export const searchQueriesRelations = relations(search_queries, ({ one, many }) => ({
    user: one(user, {
